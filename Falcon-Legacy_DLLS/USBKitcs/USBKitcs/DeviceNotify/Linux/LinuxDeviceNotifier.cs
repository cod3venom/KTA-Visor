...
// Type: USBKitcs.DeviceNotify.Linux.LinuxDeviceNotifier
  ...
  public class LinuxDeviceNotifier : IDeviceNotifier
  {
    ...
    private void OnAddRemoveEvent(object sender, AddRemoveEventArgs e)
    {
      ...
      if (onDeviceNotify == null)
        return;
      LinuxDevItem linuxDevItem = new LinuxDevItem(string.Format("usbdev{0}.{1}", (object) e.MonoUSBProfile.BusNumber, (object) e.MonoUSBProfile.DeviceAddress), e.MonoUSBProfile.BusNumber, e.MonoUSBProfile.DeviceAddress, e.MonoUSBProfile.DeviceDescriptor);
    ...
    private static bool ReadFileDescriptor(string fullPath, out byte[] deviceDescriptorBytes)
    {
      try
      {
        ...
        fileStream.Close();
        return num == UsbDeviceDescriptor.Size;
      }
      catch
      {
