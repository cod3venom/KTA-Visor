...
// Type: USBKitcs.LudnMonoLibUsb.Internal.MonoUsbTransferContext
...
using USBKitcs.MonoLibUsb.Transfer;

namespace USBKitcs.LudnMonoLibUsb.Internal
{
  internal class MonoUsbTransferContext : UsbTransfer, IDisposable
  {
    ...
    public override void Fill(IntPtr buffer, int offset, int count, int timeout)
    {
      ...
      this.mTransfer.PtrCallbackFn = Marshal.GetFunctionPointerForDelegate<MonoUsbTransferDelegate>(MonoUsbTransferContext.mMonoUsbTransferCallbackDelegate);
    ...
    public override ErrorCode Wait(out int transferredCount, bool cancel)
    {
      ...
      }, -1, false))
      {
        ...
        case 1:
          ...
          UsbError.Error(errorCode2, ret3, string.Format("Wait:Unable to cancel transfer or the transfer did not return after it was cancelled. Cancelled:{0} TransferCompleted:{1}", (object) (MonoUsbError) ret3, (object) flag), (object) this.EndpointBase);
          return errorCode2;
        default:
          int num = (int) this.mTransfer.Cancel();
          ErrorCode errorCode3 = ((int) this.EndpointBase.mEpNum & 128) > 0 ? ErrorCode.ReadFailed : ErrorCode.WriteFailed;
          this.mTransferCompleteEvent.Set();
    ...
    private static void TransferCallback(MonoUsbTransfer pTransfer) => (GCHandle.FromIntPtr(pTransfer.PtrUserData).Target as ManualResetEvent).Set();
  }
}
