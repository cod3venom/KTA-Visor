...
// Type: USBKitcs.LudnMonoLibUsb.MonoUsbDevice
...
namespace USBKitcs.LudnMonoLibUsb
{
  public class MonoUsbDevice : UsbDevice, IUsbDevice, IUsbInterface
  {
    ...
    public override UsbDevice.DriverModeType DriverMode => UsbDevice.IsLinux ? UsbDevice.DriverModeType.MonoLibUsb : UsbDevice.DriverModeType.LibUsbWinBack;
    ...
    {
      Debug.WriteLine(this.GetType().Name + ".ControlTransfer() Before", "Libusb-1.0");
      ...
      Debug.WriteLine(this.GetType().Name + ".ControlTransfer() Error:" + ((MonoUsbError) ret).ToString(), "Libusb-1.0");
      if (ret < 0)
      {
      ...
      out List<UsbConfigInfo> configInfoListRtn)
    {
      ...
      for (int configIndex = 0; configIndex < configurationCount; ++configIndex)
      {
        ...
        Debug.Print("GetConfigDescriptor:{0}", (object) configDescriptor1);
        if (configDescriptor1 != 0 || configHandle.IsInvalid)
          return UsbError.Error(ErrorCode.MonoApiError, configDescriptor1, string.Format("GetConfigDescriptor Failed at index:{0}", (object) configIndex), (object) usbDevice).ErrorCode;
        try
        {
          MonoUsbConfigDescriptor configDescriptor2 = new MonoUsbConfigDescriptor();
          Marshal.PtrToStructure<MonoUsbConfigDescriptor>(configHandle.DangerousGetHandle(), configDescriptor2);
