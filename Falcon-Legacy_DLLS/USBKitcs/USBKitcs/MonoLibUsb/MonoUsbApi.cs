...
// Type: USBKitcs.MonoLibUsb.MonoUsbApi
...
namespace USBKitcs.MonoLibUsb
{
  public static class MonoUsbApi
  {
    ...
    private static void DefaultAsyncCB(MonoUsbTransfer transfer) => (GCHandle.FromIntPtr(transfer.PtrUserData).Target as ManualResetEvent).Set();
    ...
    public static extern int GetConfiguration(
    ...
    public static extern int KernelDriverActive(
      [In] MonoUsbDeviceHandle deviceHandle,
      int interfaceNumber);

    [DllImport("libusb-1.0.dll", EntryPoint = "libusb_detach_kernel_driver")]
    public static extern int DetachKernelDriver(
      [In] MonoUsbDeviceHandle deviceHandle,
      int interfaceNumber);

    [DllImport("libusb-1.0.dll", EntryPoint = "libusb_attach_kernel_driver")]
    public static extern int AttachKernelDriver(
      [In] MonoUsbDeviceHandle deviceHandle,
      int interfaceNumber);
    ...
    public static int GetDescriptor(
      ...
      int length)
    {
      return MonoUsbApi.ControlTransfer(deviceHandle, (byte) 128, (byte) 6, (short) ((int) descType << 8 | (int) descIndex), (short) 0, pData, (short) length, 1000);
    }
    ...
    public static MonoUsbError MonoLibUsbErrorFromTransferStatus(
